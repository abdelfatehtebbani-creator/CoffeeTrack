...
### Fixed
- **`getAllReports()` وحتى استدعاءات RPC بسيطة أخرى ترجع `null` بصمت (بدون أي خطأ في السجل)**: السبب الجذري ليس البيانات (تم استبعاده بفحص شامل عبر `findUnserializableValue_`)، بل **صلاحية OAuth غير مُصرَّح بها**. الوصول إلى شيت خارجي عبر `SpreadsheetApp.openById(SPREADSHEET_ID)` يتطلب صلاحية `https://www.googleapis.com/auth/spreadsheets` الكاملة، بينما كان تفويض المشروع الأولي مبنياً على الصلاحية المحدودة الافتراضية (عندما كان الكود يتعامل فقط مع الشيت المرتبط به). عندما تفتقد نافذة الموافقة على الصلاحية الجديدة (`createOAuthDialog=true` ظهرت في رابط التنفيذ الداخلي)، يفشل الاستدعاء بصمت داخل الإطار المعزول بدل عرض شاشة موافقة. **الإصلاح**: إضافة `oauthScopes` صراحة في `appsscript.json` + إعادة تفويض المشروع يدوياً (تشغيل أي دالة من المحرر ومنح الصلاحيات) بعد كل تغيير على أي شيء يمس `SpreadsheetApp.openById()`.
