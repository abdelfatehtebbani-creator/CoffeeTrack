...
نظام متابعة تغليف القهوة (استلام → تحميص/طحن → تعبئة → منتج نهائي)، مبني بالكامل على **Google Apps Script + Google Sheets**، بواجهة عصرية ثنائية اللغة (عربي RTL / إنجليزي).

## الغرض من المشروع
تتبّع دورة القهوة الكاملة داخل المصنع:
...
كل مرحلة تتحقق تلقائياً من رصيد المرحلة السابقة، فلا يمكن تسجيل كمية أكبر مما هو متوفر فعلياً (تحقق صارم / strict validation).
...
├── EntryOperations.gs                # الدوال الخمس لإدخال البيانات + التحقق من الأرصدة
├── ReportsOperations.gs               # دوال التقارير الستة
